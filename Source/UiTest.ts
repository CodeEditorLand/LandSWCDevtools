...
export default class UiTest implements Disposable {
	...
	constructor(private readonly cargoWorkspace: Factory<CargoWorkspace>) {
		...
		workspace
			.findFiles("**/target/swc-test-results/ui/**/*.stderr")
			.then((files) => {
				for (const f of files) {
					this.diagnostics.set(f, [
						new Diagnostic(
							new Range(0, 0, 0, 0),
							"Assertion failed: output is changed",
		...
		const watcher = workspace.createFileSystemWatcher(
			"**/target/swc-test-results/ui/**/*.stderr",
		);
		...
		disposables.push(
			commands.registerTextEditorCommand(
				"swc.dev.uiTest.update",
				async (editor) => {
					// The code you place here will be executed every time your command is executed
					if (editor.document.languageId !== LANG) {
						return window.showErrorMessage(
							"This command only works for output file of ui tests",
						);
					}
					...
					try {
						...
						window.showErrorMessage(
							`Failed to update reference file: ${e}`,
						);
					}
				},
			),
	...
	private async updateReference(doc: TextDocument): Promise<void> {
		...
		const { golden, actual } = await this.resolve(
			doc.uri.with({ scheme: "file" }),
		);
	...
	private async getManifestDir(uri: Uri): Promise<string | void> {
		...
		const cargoWorkspace = await this.cargoWorkspace.get(
			Context.root(ws, "Fetching cargo workspace"),
		);
	...
	public async openDiffIfRequried(e: TextDocument): Promise<void> {
		...
		if (await exists(actual)) {
			this.diagnostics.set(Uri.file(actual), [
				new Diagnostic(
					new Range(0, 0, 0, 0),
					"Assertion failed: output is changed",
					DiagnosticSeverity.Error,
	...
	private async resolve(
		uri: Uri,
	): Promise<{ actual: string; golden: string; isGolden: boolean }> {
		const manifestDir = await this.getManifestDir(uri);
		if (manifestDir === undefined) {
			throw new Error(
				`Cannot determine cargo manifest directory for ${uri.fsPath}`,
			);
		}
		...
		if (isGolden) {
			...
			const goldenRelPath = path.relative(
				path.join("target", "ui"),
				relPath,
			);
...
export class ContentProvider implements TextDocumentContentProvider {
	...
	constructor() {
		...
		this.output = window.createOutputChannel(
			"Swc ui test - ContentProvider",
		);
	}
