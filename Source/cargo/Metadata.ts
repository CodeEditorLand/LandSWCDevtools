import { Disposable, Uri, window, workspace, WorkspaceFolder } from "vscode";
...
export function parsePkgId(pkgId: PackageId): {
	...
	if (ss.length !== 3) {
		throw new Error(
			`Cannot parse '${pkgId}' as a cargo package because it's length is ${ss.length} when splited by a space`,
		);
	}
