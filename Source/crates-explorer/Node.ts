	...
	constructor(private readonly pkg: Package) {
		...
		for (const target of pkg.targets) {
			...
			switch (kind) {
				case "test":
					this.tests.push(
						new TestFile(pkg, target.name, target.src_path),
					);
					break;

				case "bench":
					this.benches.push(
						new BenchFile(pkg, target.name, target.src_path),
					);
					break;
				...
				case "bin":
					this.binaries.push(
						new Binary(pkg, target.name, target.src_path),
					);
					break;

				case "example":
					this.examples.push(
						new Example(pkg, target.name, target.src_path),
					);
					break;
...
export class Library extends Leaf implements Buildable {
	...
	readonly label =
		this.pkg.libType == LibraryType.ProcMacro ? "Macro" : "Library";
...
abstract class TestFileLike<C extends TestFnLike> extends HasChild<C> {
	...
	getIcon(c: ExtensionContext): Icon {
		return icon(c, "rust-logo.svg");
	}
...
export class Root extends CachingFactory<Node[]> {
	constructor(readonly cargoWorkspace: Factory<CargoWorkspace>) {
