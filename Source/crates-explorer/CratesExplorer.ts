...
import RustConfigProvider, {
	RustDebugConfig,
} from "../debugger/RustConfigProvider";
import { dispose, Factory, IDisposable, ProcessBuilder } from "../util";
import { Context } from "../util/context";
import { DebugCommand, RunCommand } from "./Constants";
import { CrateSourceProvider } from "./CrateSourceProvider";
import { Crate, Node, Root } from "./Node";

export default class CratesExplorer
	implements TreeDataProvider<Node>, IDisposable
{
	public static readonly ID = "cratesExplorer";

	@dispose
	private readonly _onDidChangeTreeData: EventEmitter<Node | undefined> =
		new EventEmitter();
	public readonly onDidChangeTreeData = this._onDidChangeTreeData.event;

	@dispose
	private readonly crateSourceProvider: CrateSourceProvider =
		new CrateSourceProvider();
	...
	public constructor(
		...
		workspace.registerTextDocumentContentProvider(
			CrateSourceProvider.SCHEME,
			this.crateSourceProvider,
		);
	...
	private async run(noDebug: boolean, arg: any) {
		...
		if (!isBuildable(arg)) {
			window.showErrorMessage(
				`Invalid build target: ${arg.type} (${arg.label})`,
			);
