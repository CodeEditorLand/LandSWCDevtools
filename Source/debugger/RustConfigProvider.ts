...
import { Buildable, BuildOutput } from "../cargo/Build";
...
export default class RustConfigProvider
	implements DebugConfigurationProvider, IDisposable
{
	constructor(
	...
	async resolveDebugConfiguration(
		...
		return Context.root(ws, msg).runWith(
			async (ctx): Promise<WrappedConfig> => {
				...
				const cfg = await ctx.subTask(
					"Parsing configuration",
					async (ctx) =>
						parseRustDebugConfig(
							ctx,
							this.cargoWorkspace,
							rustc,
							debugCfg,
				...
				const type =
					mode === "gdb" || mode === "lldb" ? "cppdbg" : "cppvsdbg";
				...
				if (executables.length === 0) {
					throw new Error(
						`cargo build did not produce any executable file. `,
					);
				}
...

				{
					...
					const pkg = cargoWs.packages.find(
						(pkg) => pkg.name === crate,
					);
					if (!pkg) {
						throw new Error(
							`Cannot build unknown package ${crate}`,
						);
					}
