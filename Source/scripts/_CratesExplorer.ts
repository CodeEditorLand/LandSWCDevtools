...
const noDebug = [NodeType.BenchFiles, NodeType.TestFiles];

// -------------------------

const contexts: NodeType[] = Object.keys(NodeType).map(
	(v: string) => NodeType[<any>v] as NodeType,
);
...
function calcCond(enable: NodeType[] | CheckFn): string {
	...
	const check = isArray(enable)
		? (c: NodeType) => enable.indexOf(c) !== -1
		: enable;
...
export const debugCond = calcCond(
	(c) => runnable.indexOf(c) !== -1 && noDebug.indexOf(c) === -1,
);
